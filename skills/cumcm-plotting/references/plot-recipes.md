...
按图型读取相应章节，复制所需函数后接入真实数据。示例中的随机数、模型结果与统计量只演示接口；不是用户结果。所有代码假设已经调过 `setup_style()`，并按最终用途调整尺寸与字号。
...
sys.path.insert(0, str(Path(package_dir) / 'scripts'))  # package_dir 为当前包实际路径
...
**坑**：
- 展示观测汇总时交代样本量与误差定义；若只是各类别的单个原值，不凭空添加误差棒
- 多组比较时配色保持类别一致（同一个条件在不同图里同色）
- `barplot` 默认区间通常用 bootstrap；按统计口径明确选择 CI、SE、SD 或不绘误差，不能为提速更换其含义
