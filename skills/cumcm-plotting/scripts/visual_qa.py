"""
plotting :: visual_qa.py
=====================================
出图后的「程序自检」+「渲染预览」——自检闭环的机器那一层。

设计分工：
- **程序（本脚本）** 抓**确定性**问题：缺字乱码、文字越界裁切、刻度标签重叠。
- **AI 读图**（见 references/quality.md）抓**感知性**问题：图例压数据、
...
def audit_layout(fig, clip_tol_px: float = 2.0, overlap_tol_px: float = 1.0
    ...
    font_msgs = _draw_and_collect_glyph_warnings(fig)
    glyph_msgs = [msg for msg in font_msgs if "missing from" in msg]
    fallback_msgs = [msg for msg in font_msgs if msg not in glyph_msgs]
    if glyph_msgs:
    ...
    if fallback_msgs:
        issues.append(("WARN", "字体或字重发生回退，需读图确认：" + " | ".join(fallback_msgs[:3])))
...
def print_report(issues: list[tuple[str, str]]) -> str:
    ...
    if not issues:
        ...
        print("  >>> 仍需 AI 读图复核感知性问题（见 references/quality.md）。")
        return "PASS"
...
def _cli() -> int:
    p = argparse.ArgumentParser(description="plotting visual QA")
