"""
plotting :: profile_data.py
========================================
Exploratory data analysis for figure planning.

本脚本是 plotting"思考-绘制"工作流的第 1 步。
...
工作流位置:
    用户给数据
        ...
        → references/chart-selection.md   按数据形态查图型
        → setup_style.py
        → plot
        → inspect_image.py + references/quality.md
...
def _suggest_charts(info: dict) -> list[str]:
    """把数据特征翻译成初步图型提示；精细决策见 references/chart-selection.md。"""
    cols = info["columns"]
    ...
    if not suggestions:
        suggestions.append("数据特征不足以给出特定建议；查 references/chart-selection.md 的决策框架。")
    return suggestions


def profile_data(source, group_cols: list[str] | None = None, sheet_name=None) -> dict:
    """
    ...
    if isinstance(source, pd.DataFrame):
        ...
        if ext in (".xlsx", ".xls"):
            ...
            if sheet_name is None and len(book.sheet_names) != 1:
                raise ValueError(f"Multiple sheets: {book.sheet_names}; specify --sheet by name")
            selected = sheet_name if sheet_name is not None else book.sheet_names[0]
            with book:
                df = pd.read_excel(book, sheet_name=selected)
        elif ext in (".tsv",):
...
def _cli() -> int:
    p = argparse.ArgumentParser(description="plotting data profiler")
    p.add_argument("source", help="CSV / Excel / TSV file path")
    p.add_argument("--group", action="append", default=[],
                   help="分组列名（可多次指定形成嵌套/交叉分组）")
    p.add_argument("--sheet", help="Excel 工作表名称；多工作表时明确指定")
    p.add_argument("--json", action="store_true",
                   help="输出 JSON 而非 markdown 报告")
    args = p.parse_args()

    info = profile_data(args.source, group_cols=args.group, sheet_name=args.sheet)
    if args.json:
