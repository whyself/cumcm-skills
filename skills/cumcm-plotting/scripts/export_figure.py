...
def export_figure(
    ...
    for fmt in formats:
        if fmt in {"jpg", "jpeg"}:
            print(f"[plotting] WARNING: skipping {fmt} — "
...
def _grayscale_from(fig, basename: str, dpi: int, tight: bool,
                    pad_inches: float) -> str | None:
    """
    导出灰度预览版用于色盲安全检查。
    从内存中的同尺寸渲染转灰度，不覆盖正式 PNG。
    """
    try:
        from PIL import Image
    except ImportError:
        print("[plotting] Pillow not available; "
    ...
    fig.savefig(buffer, format="png", dpi=dpi,
                bbox_inches="tight" if tight else None, pad_inches=pad_inches)
    buffer.seek(0)
    ...
    buffer.close()
    print(f"[plotting] wrote {gray_path} (grayscale preview)")
    return gray_path
...
def _cli() -> int:
    p = argparse.ArgumentParser(description="plotting figure exporter")
    p.add_argument("cmd", choices=["demo"], help="`demo`: 跑一张演示图导出 4 种格式")
    p.add_argument("--out", default="./plot_demo",
                   help="输出 basename (默认 ./plot_demo)")
    args = p.parse_args()
    if args.cmd == "demo":
        _demo(args.out)
    return 0
