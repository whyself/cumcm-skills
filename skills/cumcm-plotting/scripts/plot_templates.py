...
Apache-2.0 plotting asset collection. This implementation is
portable, path-independent, and designed for the plotting contract.
"""
...
def plot_paired(args: argparse.Namespace) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not subject.strip() or not condition.strip():
            raise ValueError("pair ID and condition must be nonempty")
