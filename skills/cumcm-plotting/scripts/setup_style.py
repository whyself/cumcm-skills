...
def list_cjk_fonts() -> list[str]:
    ...
    for f in available:
        lower = f.lower()
        if any(k in lower for k in ("cjk", "source han", "songti", "yahei", "simhei", "simsun")):
            if f not in hits:
                hits.append(f)
    return hits
...
def configure_chinese_fonts(serif_for_zh: bool = False) -> str:
    ...
    # A registered regular face avoids silently selecting a thin-only font family.
    regular = {f.name for f in fm.fontManager.ttflist if f.weight in ("normal", "regular", 400)}
    priority = [f for f in priority if f in regular] + [f for f in priority if f not in regular]
    ...
    if chosen is None:
        ...
        for f in available:
            lower = f.lower()
            if any(k in lower for k in ("cjk", "source han", "songti", "simhei", "yahei", "kaiti", "simsun")):
                chosen = f
                break
...
def _cli() -> int:
    p = argparse.ArgumentParser(description="plotting style setup")
